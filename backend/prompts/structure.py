...
5. 所有 box 坐标使用**输入图片**的 0~1000 归一化值，格式为 {"x": 左上x, "y": 左上y, "w": 宽, "h": 高}

## 坐标约定（严格遵守）
- 参照系：输入图片，原点=图片左上角
- x、w 相对图片宽度归一化到 0~1000（1000 = 图片右边缘）
- y、h 相对图片高度归一化到 0~1000（1000 = 图片下边缘）
- 满足 0 ≤ x、y、w、h ≤ 1000，且 x+w ≤ 1000，y+h ≤ 1000
- 输出归一化数值，不要输出像素值
- 无法精确定位时 w/h 填 0（x/y 可为 0）

坐标示例（0~1000，非像素）：
"big_q_title_box": {"x": 48, "y": 43, "w": 300, "h": 98}
